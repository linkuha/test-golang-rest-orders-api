...
func (r *repo) Get(id string) (*entity.User, error) {
	query := fmt.Sprintf("SELECT id, username, password_hash FROM %s WHERE id = ?", userTableName)
...
func (r *repo) GetByUsername(username string) (*entity.User, error) {
	query := fmt.Sprintf("SELECT id, username, password_hash FROM %s WHERE username = ?", userTableName)
...
func (r *repo) Store(user *entity.User) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (username, password_hash) VALUES (?, ?) RETURNING id", userTableName)
...
func (r *repo) Update(user *entity.User) error {
	query := fmt.Sprintf("UPDATE %s SET username = ?, password_hash = ? WHERE id = ?", userTableName)
...
func (r *repo) AddFollower(userID, followerID string) error {
	query := fmt.Sprintf("INSERT INTO %s (user_id, follower_id) VALUES (?, ?) ON CONFLICT DO NOTHING", followersTableName)
